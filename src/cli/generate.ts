...
import { CATEGORIES, type Category, type PinDraft, type ProductCandidate } from '../types.js';
  ...
  for (const category of CATEGORIES) {
    ...
    if (rejected.length > 0) {
      for (const r of rejected) {
        logger.debug('Rejected candidate', { asin: r.product.asin, reason: r.reason });
      }
    }
...
function countByCategory(drafts: PinDraft[]): Record<Category, number> {
  const out: Record<Category, number> = { travel: 0, fashion: 0, beauty: 0, baby: 0 };
