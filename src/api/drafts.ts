...
export async function getDraftByAsin(asin: string): Promise<PinDraft | undefined> {
  const rows = await stores.drafts.read();
...
export async function updateDraft(asin: string, updates: DraftUpdate): Promise<PinDraft> {
  return mutateDraft(asin, (current) => {
    ...
    if (typeof updates.pinDescription === 'string' && updates.pinDescription.trim()) {
...
export async function approveDraft(asin: string, updates?: DraftUpdate): Promise<PinDraft> {
